...
const zgame = @import("zgame.zig"); // namespace
...
pub fn render(zg: *ZigGame, text: []const u8, x: i32, y: i32, scaling: u8, color: zgame.sdl.Color) !void {
...
pub fn create_text_canvas(zg: *ZigGame, text: []const u8, scaling: u8, color: zgame.sdl.Color) !_type.Canvas {
    ...
    try canvas.texture.setBlendMode(zgame.sdl.BlendMode.blend);

    try zg.renderer.setColor(zgame.sdl.Color.rgba(0, 0, 0, 0));
    var dx: i32 = 0;
    for (text) |letter| {
